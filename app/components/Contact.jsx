...
const Contact = () => {
  ...
  const onSubmit = async (event) => {
    ...
    formData.append("access_key", "d4fb4364-7501-49d7-8c93-f6c2f8499ba3");
  ...
  return (
    ...
    >
      ...
      I am eager to hear from you. Please contact me if you are interested in my services.
      </p>

      <form onSubmit={onSubmit} className='max-w-2xl mx-auto'>
        <div className='grid grid-cols-auto gap-6 mt-20 mb-8'>
        ...
        <button type='submit' className='py-3 px-8 w-max flex items-center justify-between gap-2 bg-black/80 text-white rounded-full mx-auto hover:bg-black duration-500'>
          Submit now <Image src={assets.right_white_arrow} alt='' className='w-4'/>
        </button>
        <p className='mt-4'>{result}</p> {/* Display result message here */}
      </form>
    </div>
  );
}
